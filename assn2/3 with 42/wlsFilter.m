function OUT = wlsFilter(IN,beta,  lambda, alpha, L)
...
%     beta = double(rgb2gray(B));
%     I = I./max(I(:));
%     beta = beta./max(beta(:));
%     res = wlsFilter(I,beta, 0.5);
...
dy = diff(L, 1, 1);
dy = -beta(1:r-1,:).*(lambda./(abs(dy).^alpha + smallNum));
dy = padarray(dy, [1 0], 'post');
dy = dy(:);

dx = diff(L, 1, 2); 
dx = -beta(:,1:c-1).*(lambda./(abs(dx).^alpha + smallNum));
