...
for x = 1: row 
    for y = 1:col
        if (M(x,y) ~= 0)
            result_c_a(x,y) = 1*example_c_a(x,y);
            result_c_b(x,y) = 1*example_c_b(x,y);
            result_s(x,y) = 0.8*example_s(x,y);
        end
    end 
end
% [gxM_s,gyM_s] = gradient(double(M));
% R_s = poisson_solver_function(gxM_s,gyM_s,I_s);%CHANGED
% % R_s = R_s./max(R_s(:));
% % R_s = R_s.*255;
% result_s = uint8(R_s);
% 
